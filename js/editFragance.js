function validateRequest() {
  ...
  if (reference.value.length == 0) {
    document.getElementById("registerErrorReference").innerHTML =
      "Referencia no puede ser vacío";
    validate = false;
  }
  if (marca.value.length == 0) {
    document.getElementById("registerErrorBrand").innerHTML =
      "Marca no puede ser vacío";
    validate = false;
  }
  if (precio.value.length == 0) {
    document.getElementById("registerErrorPrice").innerHTML =
      "Precio no puede ser vacío";
    validate = false;
  }
  if (cantidad.value.length == 0) {
    document.getElementById("registerErrorQuantity").innerHTML =
      "Cantidad no puede ser vacío";
    validate = false;
  }
  if (validate) {
    updateFragance();
...
function validateEmail() {
  ...
  $.ajax({
    ...
    success: function (data) {
      if (data) {
        document.getElementById("registerErrorEmail").innerHTML =
          "El email ya existe, por favor ingrese un email diferente";
        $("#email").val("");
      }
    },
    error: function () {},
